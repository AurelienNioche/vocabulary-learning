...
class TestPractice(unittest.TestCase):
    def setUp(self):
        ...
        self.vocabulary = pd.DataFrame([
            {'japanese': 'こんにちは', 'kanji': '今日は', 'french': 'bonjour', 'example_sentence': 'こんにちは、元気ですか？'},
            {'japanese': 'さようなら', 'kanji': '', 'french': 'au revoir', 'example_sentence': ''},
            {'japanese': '新しい', 'kanji': '新しい', 'french': 'nouveau/neuf', 'example_sentence': '新しい車を買いました。'}
        ])
    ...
    def test_practice_mode_commands(self, mock_input):
        ...
        self.mock_save_progress.assert_called_once()

    def test_practice_mode_correct_answer(self):
        """Test practice mode with correct answer."""
        # Mock input to first give the correct answer 'nouveau' and then quit
        with patch('builtins.input', side_effect=['nouveau', ':q']):
            practice_mode(
                self.vocabulary,
                self.progress,
                self.console,
    ...
    def test_practice_mode_incorrect_answer(self, mock_input):

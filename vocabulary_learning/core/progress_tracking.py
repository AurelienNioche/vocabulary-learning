...
def update_progress(word, success, progress, save_callback):
    ...
    if word not in progress:
        ...
        progress[word]['last_attempt_was_failure'] = progress[word].get('last_attempt_was_failure', False)
    ...
    if success:
        ...
        progress[word]['interval'] = max(0.0833, progress[word]['interval'] * 0.5)  # Minimum 5 minutes
...
def calculate_priority(word_data, active_words_count):
    ...
    priority = (
        (1 - success_rate/100) * 0.4 +     # Success rate component
        overdue_factor * 0.4 +             # Overdue component
        failure_bonus +                     # Failure bonus
        random.uniform(0, 0.1)             # Small random factor
    )
